...
export default async function ReviewerDashboard(){
  ...
  return <DashboardShell profile={profile} active="overview">
    ...
    <section className="stat-grid"><Stat label="Open requests" value={queue.length} note={`For ${profile.company_name??"your company"}`} /><Stat label="In progress" value={claimed.length} note="Claimed reviews" /><Stat label="Completed" value={completed.length} note="Available history" /><Stat label="Recorded earnings" value={`£${(total/100).toFixed(2)}`} note="£1 per completed review" /></section>
    ...
    <section className="panel"><div className="panel-heading"><div><h2>Earnings history</h2><p>Every completed review records £1.00. Paid items are marked separately.</p></div></div>{earnings.length?<div className="data-list">{earnings.map(item=><div className="data-row earning-row" key={item.id}><span className="company-logo">£</span><div className="row-main"><strong>Resume review earning</strong><small>{new Date(item.created_at).toLocaleDateString()}</small></div><span className={item.paid_at?"status status-completed":"status status-claimed"}>{item.paid_at?"Paid":"Pending payout"}</span><strong>£{(item.amount_pence/100).toFixed(2)}</strong></div>)}</div>:<div className="empty-inline"><strong>No earnings yet</strong><span>Completed reviews will appear here automatically.</span></div>}</section></>}
  </DashboardShell>
}
