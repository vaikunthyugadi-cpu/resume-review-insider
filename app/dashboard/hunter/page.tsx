...
type RequestRow = { id: string; status: "open" | "claimed" | "completed" | "cancelled"; target_role: string; created_at: string; due_at: string | null; companies: { name: string } | null; resumes: { file_name: string } | null; review_feedback: { id: string }[] | null };
...
function Status({value}:{value:string}){const label=value==="open"?"Waiting":value==="claimed"?"In progress":value==="completed"?"Completed":"Cancelled";return <span className={`status status-${value}`}>{label}</span>}
function EmptyState(){return <div className="empty-state"><span>R</span><h3>Start your first targeted review</h3><p>Choose a verified company, upload a PDF or Word resume, and receive structured insider feedback.</p><Link className="button button-primary" href="/dashboard/hunter/new">Submit a resume</Link></div>}
